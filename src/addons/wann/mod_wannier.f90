...
integer wann_ntype
integer wann_nexbnds
logical wann_add_poco
logical wann_refine
logical, allocatable :: wann_do_refine(:,:)
integer, allocatable :: wann_norb(:)
integer, allocatable :: wann_iorb(:,:,:)
integer, allocatable :: wann_iprj(:,:)
integer, allocatable :: wann_exclude_bands(:)
real(8), allocatable :: wann_eint(:,:)
real(8), allocatable :: wann_exclude_eint(:,:)
real(8) wann_unk_eint(2)
data wann_unk_eint/-100.1d0,100.1d0/
...
integer nwan,nwt,ntr,tlim(2,3),nwt0,nwtk
integer, allocatable :: iwan(:)
integer, allocatable :: idxiwan(:)
integer, allocatable :: iwt(:,:)
integer, allocatable :: wt(:,:)
...
!
integer n,j,ias,lm,ispn,itype,ilo,ierr_,prj_to_ylm,n1,n2,iw,i,j1,i1,i2,i3
integer refine_maxiter
integer itr(3)
integer ibnd(2),jdim,lwork,info
logical tbndint,tbndex
logical tbndflg1,tbndflg2,refine_bracket
real(8) d1,e1,e2,refine_step,refine_min,refine_max,refine_tol
real(8) tr(3),wann_eint_refine(2,wann_ntype)
!real(8), allocatable :: rwork(:),svec(:)
complex(8) z1
!complex(8), allocatable :: amat(:,:),umat(:,:),vmat(:,:),work(:)
logical, external :: bndint
!
refine_maxiter=100
refine_step=0.01
refine_tol=1.0d-3
wann_eint_refine(:,:)=0.0
if (wann_refine) then
  do i1=1,wann_ntype
    do i2=1,2
      if (wann_do_refine(i2,i1)) then
        do i3=1,refine_maxiter
          wanc=zzero
          tbndflg1=.true.
          tbndflg2=.true.
          ibnd(1)=nstsv+1
          ...
          do n=1,nwantot
            if (.not.wannier_lc) then
              ...
              do j=1,nstsv
                tbndint=bndint(j,eval(j),wann_eint(1,itype)+wann_eint_refine(1,itype), &
               &  wann_eint(2,itype)-wann_eint_refine(2,itype))
                tbndex=bndint(j,eval(j),wann_exclude_eint(1,itype),wann_exclude_eint(2,itype))
                if (.not.tbndex) then
                  do j1=1,wann_nexbnds
                    if (j.eq.wann_exclude_bands(j1)) tbndex=.true.
                  enddo
                endif
        ! Check if a non-included band is degenerate with an included band
  !      if (.not.tbndint) then
  !        do j1=1,nstsv
  !          if (abs(eval(j)-eval(j1)).lt.1d-6.and. &
  !              bndint(j1,eval(j1),wann_eint(1,itype),wann_eint(2,itype))) then
  !            tbndint=.true.
  !          endif
  !        enddo
  !      endif
                e1=wann_eint(1,itype)
                e2=wann_eint(2,itype)
                n1=int(e1)
                n2=int(e2)
                if (tbndint.and..not.tbndex) then
                  if (abs(dble(n1)-e1).lt.1d-5.and.abs(dble(n2)-e2).lt.1d-5 &
                      .and.n1.eq.n2) then
                    wanc(n,j)=zone
                  else
        ...
        !  Won't work with excluded bands.
                    if (.not.tbndint.and..not.tbndflg1.and.tbndflg2) then
                      if (j-1.gt.ibnd(2)) ibnd(2)=j-1
                      tbndflg2=.false.
                      tbndflg1=.true.
                    endif
  !        if (.not.tbndint) then
  ...
  !        endif
        ...
        ! <psi_k(r)|g(r-T)>=<psi(r+T)|g(r)>=e^{-ikT}<psi(r)|g(r)>
                      wanc(n,j)=wanc(n,j)+z1*d1*exp(-zi*dot_product(vpc,tr))
                    endif !bndint
                  enddo
                endif
              enddo !i
        ...
        !endif
  !      if (wannier_lc) then
  !        jdim=ibnd(2)-ibnd(1)+1
  !        allocate(amat(nwantot,jdim),svec(nwantot))
  !        allocate(umat(nwantot,nwantot),vmat(jdim,jdim))
  !        lwork=2*(nwantot+jdim)
  !        allocate(work(lwork),rwork(5*nwantot))
  !        i=0
!        do j=ibnd(1),ibnd(2)
!          i=i+1
!          do n=1,nwantot
...
!          enddo
!        enddo
!        call zgesvd('A','A',nwantot,jdim,amat(1,1),nwantot,svec(1), &
!                  & umat(1,1),nwantot,vmat(1,1),jdim,work(1),lwork,rwork(1),info)
!      endif
!if (mpi_grid_root()) then
!  write(*,*)'info=',info
!  write(*,*)'svec='
...
!      if (wannier_lc) then
!        do n=1,nwantot
!          do n1=1,wanlc_norb(n)
!            iw=wanlc_iorb(1,n1,n)
!            if (iw.le.0) then
!              i=0
!              do j=ibnd(1),ibnd(2)
!                i=i+1
...
!if (mpi_grid_root()) write(*,*)wanc
  ...
  !      if (mpi_grid_root()) then
  !        write(*,*)'i1,i2,i3,refine_bracket,wann_eint_refine(i2,i1),ierr_' &
  !       &  ,i1,i2,i3,refine_bracket,wann_eint_refine(i2,i1),ierr_
  !      endif
          if (i3.eq.1) then
            if (ierr_.ne.0) goto 101
            e1=wann_eint(i2,i1)
            n1=int(e1)
            if (abs(dble(n1)-e1).lt.1d-5) exit
            refine_min=0.0
            refine_max=refine_step
            wann_eint_refine(i2,i1)=refine_max
            refine_bracket=.false.
          else
            if (refine_bracket) then
              if (ierr_.eq.0) then
                refine_min=wann_eint_refine(i2,i1)
                wann_eint_refine(i2,i1)=refine_min+(refine_max-refine_min)/2
              else
                refine_max=wann_eint_refine(i2,i1)
                wann_eint_refine(i2,i1)=refine_min+(refine_max-refine_min)/2
              endif
            else
              if (ierr_.eq.0) then
                refine_min=refine_max
                refine_max=refine_max+refine_step
                wann_eint_refine(i2,i1)=refine_max
              else
                refine_bracket=.true.
                wann_eint_refine(i2,i1)=refine_min+(refine_max-refine_min)/2
          ...
          if (refine_max-refine_min.lt.refine_tol) then
            if (ierr_.eq.0) then
              wann_eint_refine(i2,i1)=refine_min
              exit
            else
              wann_eint_refine(i2,i1)=refine_min
            endif
...
if (vrinmt2(x,is,ia,ntr,ir0,vr0,dr)) then
  ...
  enddo !l
  zm1=zzero
  if (present(ikloc_in)) then
    ik=mpi_grid_map(nkptnr,dim_k,loc=ikloc_in)
    zt1=exp(zi*dot_product(vkcnr(:,ik),vtc(:)))*wkptnr(ik)
    do ispn=1,nspinor
      if (present(n)) then
        zm1(:,:,ispn)=zm1(:,:,ispn)+zt1*wann_unkmt(:,:,ias,ispn,n,ikloc_in)
      else
  ...
  do ikloc=1,nkptnrloc
    ...
    enddo
  enddo
  call mpi_grid_reduce(zm1(1,1,1),lmmaxapw*nufrmax*nspinor,dims=(/dim_k/))
  endif
  do ispn=1,nspinor
    do lm=1,lmmaxapw
      l=lm2l(lm)
...
! TODO: check if this can be optimized by switching order of indexes
    do ispn=1,nspinor
      ...
      zt2=expigr(m_igkignr(ig,ik))
        if (present(n)) then
          zm2(ispn)=zm2(ispn)+zt2*wann_unkit(ig,ispn,n,ikloc_in)
        else
...
! TODO: check if this can be optimized by switching order of indexes
    ...
    enddo
  enddo
  call mpi_grid_reduce(wanval(1),nspinor,dims=(/dim_k/))
  endif
endif
return
end subroutine
